---
marp: true
paginate: true
---
<style>
:root {
  font-family: 'Noto Sans JP';    
  color: #444;
}
:root ol {
  list-style-type: decimal;
}
:root h1, h2, h3, h4, h5, h6{
  font-family: 'Noto Sans JP Black';
  color: #2277cc;
}
pre, code {
  font-family: 'Roboto mono', 'Noto Sans JP';
  line-height: 1.5;
}
</style>
# ライブコーディングって何だろう?

田所 淳

---

# イントロダクション

---

## イントロダクション

**Algorave Generation | Resident Advisor**
https://youtu.be/S2EZqikCIfY

![height:360](https://i.ytimg.com/vi/S2EZqikCIfY/maxresdefault.jpg)

---

# Live coding

---

## Live coding
### いろいろな言い方が

* Live programming
* Interactive programming
* On-the-fly programming
* Performative programming
* ascii music...

---

## Live coding

今回は「ライブ」であることを重視して
**ライブコーディング (Live coding)** を採用

まず「ライブ」の意味について

---

# Live - コードのライブ編集

---

## ライブ - コードのライブ編集

**インタプリタ型のプログラミング言語**

* LISP (1962)
* LOGO (1965) - Turtle graphics

---

[Turtle graphics](https://el.media.mit.edu/logo-foundation/what_is_logo/logo_primer.html)

![height:480](https://cdn-blog.adafruit.com/uploads/2019/01/logo-wide-1.png)

---

## ライブ - コードのライブ編集

**プログラミング開発環境の新しい潮流**

[Bret Victor - Inventing on Principle](https://vimeo.com/906418692?share=copy#t=150.357)

![height:360](https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSL3-_dj3TtwAfQk5O3sDEJVIR6fUWayfQLOw&s)



---

## ライブ - コードのライブ編集

[**p5.js editor**](https://editor.p5js.org/)

![height:380](https://repository-images.githubusercontent.com/57940487/285cbed2-c56a-4007-a575-535d2275f41a)

---

## ライブ - コードのライブ編集

[**Kodelife**](https://hexler.net/kodelife)

![height:380](https://hexler.net/gfx/_news/kodelife-compute-mandelbrot.png)

---

# Live - ライブパフォーマンスとしてのコーディング

---

## Live - ライブパフォーマンスとしてのコーディング

- ライブ」のもう一つの意味
  - ライブパフォーマンスとしてのコーディング

- 音楽のライブパフォーマンスにおいて、様々な試みが行なわれてきた
- 「ライブエレクトロニクス (live electronic music)」

---

## Live - ライブパフォーマンスとしてのコーディング

**1990’s Laptronica (Live electronic music)**
Farmers Manual

![height:360](https://upload.wikimedia.org/wikipedia/commons/1/15/Farmers_Manual_2002.jpg)

---

## Live - ライブパフォーマンスとしてのコーディング

**この流れは今でも**
例: Mark Fell - Mark FELL_Concert PRESENCES électronique 2014
https://youtu.be/s15wdk2xQik

![height:360](https://i.ytimg.com/vi/s15wdk2xQik/maxresdefault.jpg)

---

## Live - ライブパフォーマンスとしてのコーディング

**しかしそこには問題が…**
実際になにをしているの? 本当に演奏しているの?

**2つの映像を比較**
Kraftwerk - Pocket Calculator
https://youtu.be/UZiTr4hwHXI?si=ZkCJfKEeqSVAL-jn
https://www.youtube.com/watch?v=K_pxAVULNEw

---

## Live - ライブパフォーマンスとしてのコーディング

**“Show Us Your Screens” (スクリーンを晒せ)**
ライブコーディングのキーコンセプト

https://vimeo.com/20241649?share=copy

---

## Live - ライブパフォーマンスとしてのコーディング

**Slub (2000) : Adrian Ward, Alex McLean, Dave Griffith**
スクリーンにプログラミングの様子を投射しながらパフォーマンスする試みを開始
http://slub.org/

![height:360](https://upload.wikimedia.org/wikipedia/commons/thumb/d/d1/Slub_live_Roebuck.jpg/1200px-Slub_live_Roebuck.jpg)

---

## Live - ライブパフォーマンスとしてのコーディング

**SLUB Live Coding performance Aarhus Kunsthal**
Alex McLean, Dave Griffiths
https://youtu.be/Z-8DX7g2zEc

---

## Live - ライブパフォーマンスとしてのコーディング

**Alex McLean (Yaxu) live on DOMMUNE**
2018年、来日したAlex McLeanのDOMMUNEでのパフォーマンス
https://www.youtube.com/watch?v=dIpzU71LAQQ

![height:360](https://i.ytimg.com/vi/dIpzU71LAQQ/maxresdefault.jpg)



---

## Live - ライブパフォーマンスとしてのコーディング

**NIME 2016 'Barely a Piano' by Andrew Sorensen**
https://vimeo.com/176422744

---

## Live - ライブパフォーマンスとしてのコーディング

**Renick Bell, live @ Tama Art University**
Nov. 27 2013, Tokyo Japan
https://youtu.be/BvXUK6FXPu8


---

## ライブコーディングのコミュニティー
- TOPLAP (2004) : The live coding community
- https://toplap.org/

![height:360](https://raw.githubusercontent.com/toplap/awesome-livecoding/images/toplap-circle.png)

---

## Algorave

- Algorave = algorithm + rave
- ライブコーディングによるパフォーマンスを主体にしたパーティー
- https://algorave.com/

![height:360](https://www.cityofmediaarts.de/wp-content/uploads/2022/03/Algorave-1024x639.jpg)

---

## TOPLAP Manufest

http://toplap.org/wiki/ManifestoDraft

- Give us access to the performer's mind, to the whole human instrument.
- Obscurantism is dangerous. **Show us your screens.**
- Programs are instruments that can change themselves
- The program is to be transcended - Artificial language is the way.
- Code should be seen as well as heard, underlying algorithms viewed as well as their visual outcome.
- Live coding is not about tools. Algorithms are thoughts. Chainsaws are tools. That's why algorithms are sometimes harder to notice than chainsaws.
25

---

## それでは実際にLive codingを体験してみましょう!